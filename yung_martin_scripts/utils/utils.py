...
def standardize_dateformat(df: DataFrame, time_feature: str, start_idx=1, 
                           end_idx=10, replace_char='/', retain_in_day_time=False):
    ...
    Args:
        ...
        - start_idx (int): Starting character-index for replacements (FROM)
        - end_idx (int): Ending character-index for replacements (TO)
        - retain_in_day_time (bool): True: Keeps hh:mm:ss | False: Discards hh:mm:ss 
    ...
    try:
        ...
        df = df.withColumn(
            time_feature, regexp_replace(col(time_feature), replace_char, to_char)
        )

        if retain_in_day_time:
...
def print_unique_values(df, target_feature='trawling', label=1, unique_feature='MMSI'):
    unique_value = df[df[target_feature]==label][unique_feature].unique()
    print(unique_value)
    return unique_value