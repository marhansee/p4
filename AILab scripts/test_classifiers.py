...
def plot_confusion_matrix(y_true, y_pred, save_img_path):
    ...
    cm = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis] * 100 # Make to percentage
...
def main():
    ...
    # Make input sequences with corresponding target
    ...
    # ONNX model path
    onnx_model_path = f'models/classifiers/{args.snapshot_name}.onnx'  # Replace with the actual ONNX model path
