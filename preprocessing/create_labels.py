...
def add_future_lags(df, horizon=120):
    ...
    # Drop existing timestamp_epoch if it exists
    if "timestamp_epoch" in df.columns:
        df = df.drop("timestamp_epoch")

    # Use 'ts' to generate ordering key
    df = df.withColumn("timestamp_epoch", F.unix_timestamp("ts").cast("long"))

    # Define window ordered by timestamp within each vessel
    window = Window.partitionBy("MMSI").orderBy("timestamp_epoch")

    # Create lagged (future) columns for Latitude and Longitude
    lagged_cols = []
    for i in range(1, horizon + 1):
        lagged_cols.append(F.lag("Latitude", -i).over(window).alias(f"future_lat_{i}"))
        lagged_cols.append(F.lag("Longitude", -i).over(window).alias(f"future_lon_{i}"))

    # Select all original columns + lagged ones
    df = df.select("*", *lagged_cols)

    # Cache and force evaluation to avoid plan explosion later
    df = df.cache()
    df.count()
